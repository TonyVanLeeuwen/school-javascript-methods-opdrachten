...
let emailadresses = ['n.eeken@novi.nl', 'n.stuivenberg@novi.nl', 'm.vandergeest@novi-education.nl', 'a.wiersma@novi.nl'];
...
function changeMailAdress(addresses, oldMail, newMail) {
    for (let i = 0; i < addresses.length; i++) {
        if (addresses[i] === oldMail) {
            addresses[i] = newMail;
        }
    }
    return addresses;
...
emailadresses = changeMailAdress(emailadresses, "m.vandergeest@novi-education.nl", "m.vandergeest@novi.nl"); //emailadresses een let gemaakt, anders werkt deze niet
